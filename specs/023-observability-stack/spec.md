...
- [DECISION 2026-09-13] Distributed tracing is now IN SCOPE: an HTTP trace spine (gateway → api → Npgsql) with OpenTelemetry traces exported via OTLP/HTTP and trace ids correlated into Loki logs. The "tracing becomes relevant when services split" line below no longer holds — finance-sentry already has the shape (OTel metrics, a YARP gateway, Serilog → Loki, Hangfire) that makes a trace spine worth having now. Frontend (browser) telemetry remains out of scope.
