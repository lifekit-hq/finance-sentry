...
| Unpin accepts two forms | The derived key first, then the raw trimmed/lowercased input as a key | Normalization is not a fixed point over every key it emits — pinning `"*MOBI TOP-UP 0857860057"` stores `mobile top-up 0057`, which re-derives to `mobile top-up`. The listing advertises the key and list-then-unpin is the only flow an MCP caller has, so a derived-key-only lookup would 404 on the pin it just showed |
