...
**Broad-universe opportunity scanning (#558) — DONE, enabled in the API host.** The scan universe is no longer held-positions ∪ watchlist. `Radar:BroadUniverseEnabled` (set `true` in `backend/src/FinanceSentry.API/appsettings.json`; override anywhere with the env var `Radar__BroadUniverseEnabled=true`) makes `RadarUniverseService` append the checked-in S&P 500 seed as `UniverseKind.IndexConstituent` members — ownership kinds still win, so a constituent you hold stays a `Holding`. The existing nightly `radar-ingestion` job fetches their bars; `Radar:BroadUniverseMaxIngestPerRun` (275, the code default) is the per-run upstream fetch budget, spent core-members-first and then on the least-fresh constituents, so a full rotation of the 503-name seed takes two runs — just inside `Radar:FreshnessMaxTradingDays` (2), with no margin and no watchdog, so a skipped ingestion run quietly shrinks the ranked universe. Constituents are exempt from `RadarFreshnessWatchdogJob` (rotation makes them stale by design). `OpportunityScanJob` then ranks the whole ingested universe by combined quality × momentum (`Opportunity:ScanQualityWeight`, EDGAR fan-out bounded by `max(Opportunity:ScanQualityShortlistSize, Opportunity:ScanMaxNominationsPerRun)`). **Turning it off** is the same key set to `false`: the next universe sync deactivates the constituent members and the scanner falls back to the book. The seed list (`Modules.Research/Infrastructure/Resources/sp500-constituents.json`) is regenerated from the public constituents dataset and every symbol verified against the Yahoo chart endpoint — symbols are stored in Yahoo form (`BRK-B`, not `BRK.B`); never hand-edit it. Spec: `specs/20260910-001634-broad-universe-scan/`.
